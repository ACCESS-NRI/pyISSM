...
md.materials = pyissm.model.materials.materials('litho', 'ice')
...
# Initial velocity
x = pyissm.wrappers.tools.arch_read('../Data/SquareSheetConstrained.arch', 'x')
y = pyissm.wrappers.tools.arch_read('../Data/SquareSheetConstrained.arch', 'y')
vx = pyissm.wrappers.tools.arch_read('../Data/SquareSheetConstrained.arch', 'vx')
vy = pyissm.wrappers.tools.arch_read('../Data/SquareSheetConstrained.arch', 'vy')
index = pyissm.wrappers.tools.arch_read('../Data/SquareSheetConstrained.arch', 'index').astype(int)
md.initialization.vx = np.array(pyissm.wrappers.tools.interp_from_mesh_to_mesh_2d(index, x, y, vx, md.mesh.x, md.mesh.y)).reshape(-1, 1)
md.initialization.vy = np.array(pyissm.wrappers.tools.interp_from_mesh_to_mesh_2d(index, x, y, vy, md.mesh.x, md.mesh.y)).reshape(-1, 1)
vx = None
vy = None
x = None
y = None
...
md.materials.rheology_B = pyissm.model.materials.paterson(md.initialization.temperature)
