...
# Parameterise model
md = pyissm.model.mesh.triangle(pyissm.model.Model(), '../assets/Exp/Square.exp', 300000.)
...
SPC_cold = np.nan * np.ones((md.mesh.numberofvertices, ))
SPC_warm = np.nan * np.ones((md.mesh.numberofvertices, ))
SPC_cold[pos] = Ts
SPC_warm[pos] = Tsw
...
i4 = len(md.results.TransientSolution.steps) - 1
