...
class issmsettings(class_registry.manage_state):
    ...
    This class contains general settings and configuration parameters for the ISSM framework.
    It controls output frequency, I/O behavior, memory management, solver settings, and other
    system-wide parameters that affect the overall behavior of ISSM simulations.
    ...
    def __repr__(self):
        ...
        s += '{}\n'.format(class_utils._field_display(self, 'results_on_nodes', "list of output for which results will be output for all the nodes of each element, Use 'all' for all output on nodes."))
        s += '{}\n'.format(class_utils._field_display(self, 'io_gather', 'I / O gathering strategy for result outputs (default 1)'))
        s += '{}\n'.format(class_utils._field_display(self, 'lowmem', 'is the memory limited ? (0 or 1)'))
        s += '{}\n'.format(class_utils._field_display(self, 'output_frequency', 'number of time steps between two saves (e.g., 5 means that results are only saved every 5 time steps)'))
